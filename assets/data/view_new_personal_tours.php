...
try {
    ...
    $stmtTour = $pdo->prepare("SELECT * FROM createTours WHERE id = :id");
    $stmtTour->execute(["id" => $id]);
    $tour = $stmtTour->fetch(PDO::FETCH_ASSOC);

    // Récupérer les détails du classeur
    $stmtClasseur = $pdo->prepare("SELECT * FROM classeur WHERE id = :id");
    $stmtClasseur->execute(["id" => $id]);
    $classeur = $stmtClasseur->fetch(PDO::FETCH_ASSOC);

    // Récupérer les détails du contact
    $stmtContact = $pdo->prepare("SELECT * FROM contact WHERE id = :id");
    $stmtContact->execute(["id" => $id]);
    $contact = $stmtContact->fetch(PDO::FETCH_ASSOC);

    // Vérifier si au moins un des enregistrements existe
    if (!$tour && !$classeur && !$contact) {
...
<body class="bg f">
    ...
    <main class=" shadow bg-white rounded p-3">
        ...
        <div class="container mx-auto">
            <div class="text-start p-4 mt-4 bg-danger bg-opacity-10 shadow-sm">
                <h2>MESSAGE MAIL <br>
                    <p class="fs-5"> NOM : <u><i class="text-primary-emphasis"><?= htmlspecialchars($tour["name"]) ?></i></u> <br>
                        DOSSIER N°: <u><i class="text-primary-emphasis"><?= htmlspecialchars($tour["id"]) ?></i></u>
                    </p>
            ...
            <div class=" row">
                ...
                <div class="col-lg-6">
                    <div class=" bg-light shadow-sm m-3" id="nav">
                        <p><strong>Nom :</strong> <br>
                            <?= htmlspecialchars($tour["name"]) ?>
                        </p>
                    </div>

                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Email :</strong> <br>
                            <?= htmlspecialchars($tour["email"]) ?>
                        </p>
                    </div>

                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Adresse :</strong> <br>
                            <?= htmlspecialchars($tour["adresse"]) ?>
                        </p>
                    </div>

                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Téléphone :</strong> <br>
                            <?= htmlspecialchars($tour["phone"]) ?>
                        </p>
                    </div>

                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Date d'Arrivée :</strong> <br>
                            <?= htmlspecialchars($tour["arrival"]) ?>
                        </p>
                    </div>
                    ...
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Adultes :</strong> <br>
                            <?= htmlspecialchars($tour["adults"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Enfants :</strong> <br>
                            <?= htmlspecialchars($tour["children"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Chambre :</strong> <br>
                            <?= htmlspecialchars($tour["room"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Style de voyage :</strong> <br>
                            <?= htmlspecialchars($tour["travelstyle"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Vie sauvage :</strong> <br>
                            <?= htmlspecialchars($tour["wildlife"]) ?>
                        </p>
                    </div>
                </div>
                ...
                <div class="col-lg-6">
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Parc :</strong> <br>
                            <?= htmlspecialchars($tour["park"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Route :</strong> <br>
                            <?= htmlspecialchars($tour["road"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Air :</strong> <br>
                            <?= htmlspecialchars($tour["air"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Nourriture :</strong> <br>
                            <?= htmlspecialchars($tour["food"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Guide :</strong> <br>
                            <?= htmlspecialchars($tour["guide"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Condition physique :</strong> <br>
                            <?= htmlspecialchars($tour["fitness"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Capacité :</strong> <br>
                            <?= htmlspecialchars($tour["ability"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Infos supplémentaires :</strong> <br>
                            <?= htmlspecialchars($tour["autreinfo"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Budget :</strong> <br>
                            <?= htmlspecialchars($tour["budget"]) ?>
                        </p>
                    </div>
                    <div class=" bg-light shadow-sm m-3">
                        <p><strong>Avis :</strong> <br>
                            <?= htmlspecialchars($tour["avis"]) ?>
                        </p>
                    </div>
        ...
        <div class="text-center m-5 ">
            <a href="admin_fhltravel.php" class="btn colorbouton rounded-pill ">Retour</a>
